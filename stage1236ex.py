...
# 触发设置
right_char_in_trigger = "没错！都是你的错羽泽度芽！"
right_char_out_trigger = "out"
title_trigger = "仅凭镜子存在与此，我歌镜明时"
bgm_trigger = "我们也有个惊喜要送给你"

left1_char_out_trigger = ""
left2_char_out_trigger = "——什么？！"
boss_num=7
...
if boss_num==8:
    ...
    right_color = (142, 186, 205)
elif boss_num==1:
    right_color = (183, 155, 122)



#导入路径设置

background_path=f"image/background/bg_{boss_num}_02.png"
title_path=f"image/title/title_{boss_num}.png"
bgm_path=f"image/bgm/bgm_{boss_num}_1.png"
text_path=f"text/talk_{boss_num}_0_1.txt"
...
dialogue_font = pygame.font.Font(dialogue_font_path, 13)
...
def load_resource():
    ...
    if boss_num == 8:
        resources["board"] = pygame.image.load("image/else/game_board_1_ex.png").convert_alpha()
    else:
...
# ====================== 左侧双角色配置（移到右侧角色移动实例前，确保变量定义顺序正确）======================
LEFT_CHAR1_POS = (-320, 200)    # 左1角色基础位置（编号00）
LEFT_CHAR2_POS = (25, 150)     # 左2角色基础位置（编号10）
...
right_sprite_mover = SpriteMover(
    start_x=550,
    enter_target_x=70 if boss_num==7 else 170,
    leave_target_x=550,
    enter_speed=25,
    leave_speed=25
)
...
def main():
    ...
    last_sprite_idx = {00: 0, 10: 0,boss_num: 0}
    ...
    while running:
        ...
        if speaker_idx == 00:
            last_sprite_idx[00] = current_sprite_idx  # 左1（00）说话，更新左1立绘索引
        elif speaker_idx == 10:
            last_sprite_idx[10] = current_sprite_idx  # 左2（10）说话，更新左2立绘索引
        elif speaker_idx == boss_num:
        ...
        char0_orig, char0_dark = load_character_sprite(
            char_idx=00,
            sprite_idx=last_sprite_idx[00],
            char_sprites_cache=char_sprites_cache
        )

        # 左2角色（编号10）：调用通用函数加载
        char2_orig, char2_dark = load_character_sprite(
            char_idx=10,
            sprite_idx=last_sprite_idx[10],
            char_sprites_cache=char_sprites_cache
        )
        ...
        # 更新上一次左侧说话者（左侧角色为00和10）
        if speaker_idx in (00, 10):
            last_left_speaker = speaker_idx

        if flag == 0:
            left1_sprite_mover.trigger_enter()
            left2_sprite_mover.trigger_enter()
            flag=1

        if left2_char_out_trigger in content and not left2_sprite_mover.leave_done:
            left2_sprite_mover.trigger_leave()
        ...
        if speaker_idx == boss_num:  # 右侧（8）讲话
            if last_left_speaker == 00:#上次为左1（00）讲话
                screen.blit(char2_dark, (left2_sprite_mover.x, LEFT_CHAR2_POS[1]))
                screen.blit(char0_dark, (left1_sprite_mover.x, LEFT_CHAR1_POS[1]))
                screen.blit(original_sprite1, (right_sprite_mover.x, 175))
            elif last_left_speaker == 10:#上次为左2（10）讲话
                screen.blit(char0_dark, (left1_sprite_mover.x, LEFT_CHAR1_POS[1]))
                screen.blit(char2_dark, (left2_sprite_mover.x, LEFT_CHAR2_POS[1]))
                screen.blit(original_sprite1, (right_sprite_mover.x, 175))
            else:#上次右侧讲话
            ...
            if speaker_idx == 00:  # 左1（00）讲话：右 → 左2 → 左1
                # 左2（暗）
                screen.blit(char2_dark, (left2_sprite_mover.x, LEFT_CHAR2_POS[1]))
                # 左1（亮+上移）
                char0_speak_pos = (left1_sprite_mover.x, LEFT_CHAR1_POS[1]+ LEFT_SPEAKER_OFFSET)
                screen.blit(char0_orig, char0_speak_pos)
            elif speaker_idx == 10:  # 左2（10）讲话：右 → 左1 → 左2
                # 左1（暗）
                screen.blit(char0_dark, (left1_sprite_mover.x, LEFT_CHAR1_POS[1]))
                # 左2（亮+上移）
                char2_speak_pos = (left2_sprite_mover.x, LEFT_CHAR2_POS[1] + LEFT_SPEAKER_OFFSET)
                screen.blit(char2_orig, char2_speak_pos)
