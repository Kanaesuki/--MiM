...
char6_leave_trigger = "——什么？！"
char10_leave_trigger = "——什么？！"
char4_enter_trigger = "啊……你是……羽泽度芽……"
char6_enter_trigger = "怎么又碰到你们了"
right_char_out_trigger = "out"
title_trigger = "我一直在寻找你"
bgm_trigger = "几百年间的孤独"
...
RIGHT_CHAR_POS_Y = 175  # 右侧角色（6、4）基础Y坐标
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if len(parts) >= 4:
                ...
                # 根据全局触发变量，记录对应台词的索引
                if char_idx == CHAR00_IDX and content == char6_leave_trigger:
                    trigger_indices["char6_leave"] = len(dialogue)
                if char_idx == CHAR00_IDX and content == char10_leave_trigger:
                    trigger_indices["char10_leave"] = len(dialogue)
                if char_idx == CHAR4_IDX and content == char4_enter_trigger:
                    trigger_indices["char4_enter"] = len(dialogue)
                if char_idx == CHAR6_IDX and content == char6_enter_trigger:
...
def main():
    ...
    while running:
        ...
        # ====================== 初始入场触发 =======================
        if flag == 0:
            left1_sprite_mover.trigger_enter()  # 00入场
            char4_sprite_mover.trigger_enter()  # 6入场
            flag = 1
        ...
        # 2. 编号10（自机赤红）离场触发
        if current_line == trigger_indices["char10_leave"] and not left2_sprite_mover.leave_done:
